...
const stats = [
  { label: 'Рецепти', value: 12, icon: '📝' },
  { label: 'Одобрени', value: 8, icon: '✅' },
  { label: 'Чакащи', value: 3, icon: '⏳' },
  { label: 'Отхвърлени', value: 1, icon: '❌' },
]
...
<template>
  <div class="p-8">
    <!-- Welcome -->
    <div class="mb-8">
      <h2 class="text-[1.75rem] font-bold text-brand-900 mb-1">Добре дошъл, Иванe! 👋</h2>
      <p class="text-brand-600 text-[0.95rem]">Ето какво се случва с твоите рецепти.</p>
    </div>
    ...
    <div class="grid grid-cols-4 gap-4 mb-8">
      <div
        v-for="stat in stats"
        :key="stat.label"
        class="bg-white rounded-xl p-5 flex items-center gap-4 border border-brand-100 shadow-sm"
      >
        <span class="text-2xl">{{ stat.icon }}</span>
        <div class="flex flex-col">
          <span class="text-2xl font-bold text-brand-900">{{ stat.value }}</span>
    ...
    <div class="grid grid-cols-[300px_1fr] gap-6">
      <!-- Activity -->
      <div class="bg-white rounded-xl p-6 border border-brand-100 shadow-sm">
